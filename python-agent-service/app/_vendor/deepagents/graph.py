...
# =============================================================================
# `graph.py` is vendored; SecManus changes are wrapped in "SECMANUS PATCH" blocks.
#
# When upgrading / merging upstream DeepAgents:
#   1. Diff this file against the new upstream `graph.py`.
#   2. Re-apply every block marked `# --- SECMANUS PATCH: ... ---`.
#   3. See `middleware/SECMANUS_VENDOR_PATCHES.md` → section **graph.py**.
#
# Related (non-vendor): `app/context_budget/summarization.py`,
#   `app/parsers/deepagents_stream_adapter.py` (`configurable["_context_meter"]`).
# =============================================================================
...
from app._vendor.deepagents._models import get_model_identifier, get_model_provider, resolve_model
from app._vendor.deepagents._version import __version__
from app._vendor.deepagents.backends import StateBackend
from app._vendor.deepagents.backends.protocol import BackendFactory, BackendProtocol
from app._vendor.deepagents.middleware._tool_exclusion import _ToolExclusionMiddleware
...
from app._vendor.deepagents.middleware.patch_tool_calls import PatchToolCallsMiddleware
from app._vendor.deepagents.middleware.permissions import FilesystemPermission, _PermissionMiddleware
from app._vendor.deepagents.middleware.skills import SkillsMiddleware
...
from app._vendor.deepagents.middleware.summarization import create_summarization_middleware

# --- SECMANUS PATCH: main-graph summarization uses provider ContextMeter OR-trigger ---
# Import from `app.context_budget.summarization` (not `app.context_budget` package root)
# to avoid a circular import with this module during package init.
from app.context_budget.summarization import create_secmanus_summarization_middleware

# --- end SECMANUS PATCH (summarization imports) ---

from app._vendor.deepagents.profiles import _get_harness_profile, _HarnessProfile
    ...
    *,
    ...
    deepagent_middleware.extend(
        [
            ...
            # --- SECMANUS PATCH: main agent only — gp / named subagents stay on vanilla
            #     `create_summarization_middleware` so they do not read parent `_context_meter`.
            create_secmanus_summarization_middleware(model, backend),
            # --- end SECMANUS PATCH (main summarization middleware) ---
            PatchToolCallsMiddleware(),
        ]
    )
