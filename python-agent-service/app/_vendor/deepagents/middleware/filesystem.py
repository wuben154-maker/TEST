...
from app._vendor.deepagents.backends import CompositeBackend, StateBackend
...
def _subagent_skill_file_read_metadata(file_path: str) -> dict[str, str] | None:
    """Return telemetry fields for subagent skill SKILL.md reads."""
    prefix = "/subagent-skills/"
    suffix = "/SKILL.md"
    if not file_path.startswith(prefix) or not file_path.endswith(suffix):
        return None

    parts = [part for part in file_path.strip("/").split("/") if part]
    if len(parts) < 4 or parts[0] != "subagent-skills" or parts[-1] != "SKILL.md":
        return None
...
class FilesystemMiddleware(AgentMiddleware[FilesystemState, ContextT, ResponseT]):
    ...
    Example:
        ...
        from app._vendor.deepagents.backends import StateBackend, StoreBackend, CompositeBackend
    ...
    def _create_read_file_tool(self) -> BaseTool:  # noqa: C901
        ...
        def _record_subagent_skill_file_read(
            validated_path: str,
            ...
            metadata = _subagent_skill_file_read_metadata(validated_path)
            if metadata is None:
                return
            ...
            if isinstance(read_result, ReadResult):
                if read_result.error or read_result.file_data is None:
                    status = "error"
                    error = read_result.error or "no data returned"
        ...
        def sync_read_file(
            ...
            _record_subagent_skill_file_read(validated_path, read_result, runtime.tool_call_id, offset, limit)
