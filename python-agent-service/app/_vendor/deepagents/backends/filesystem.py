...
# --- SECMANUS PATCH: fmt_timestamp DI �?default formatter (start) ---
def _default_fmt_mtime(ts: float) -> str:
    """Fallback: plain UTC wall-clock string, seconds precision, no app dependency."""
    return datetime.fromtimestamp(ts, tz=timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
# --- end SECMANUS PATCH (fmt_timestamp DI �?default formatter) ---
...
class FilesystemBackend(BackendProtocol):
    ...
    def __init__(
        ...
        # --- SECMANUS PATCH: fmt_timestamp DI (start) ---
        fmt_timestamp: Callable[[float], str] | None = None,
        # --- end SECMANUS PATCH (fmt_timestamp DI) ---
    ) -> None:
        ...
        # --- SECMANUS PATCH: fmt_timestamp DI (start) ---
        self._fmt_mtime: Callable[[float], str] = fmt_timestamp if fmt_timestamp is not None else _default_fmt_mtime
        # --- end SECMANUS PATCH (fmt_timestamp DI) ---
                            ...
                            results.append(
                                {
                                    ...
                                    "is_dir": False,
                                    "size": int(st.st_size),
                                    "modified_at": self._fmt_mtime(st.st_mtime),  # noqa: DTZ006  # Local filesystem timestamps don't need timezone
                                }
                            )
                        except OSError:
