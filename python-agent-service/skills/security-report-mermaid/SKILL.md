---
name: security-report-mermaid
display_name: Security Report (Markdown + Mermaid)
description: Professional markdown reports with Mermaid diagrams for SecManus workspace output, security analyses, and executive-style briefs. Load before writing user-facing ## SM_FULL_REPORT bodies or long markdown conclusions. Includes full style guides under references/ and templates under templates/.
license: Apache-2.0
version: 1.0.0
author: SecManus (bundle); upstream content — Superior Byte Works / agent-project (see NOTICE.txt)
triggers:
  - report
  - markdown
  - mermaid
...
priority: 5
metadata:
  secmanus-skill-package: security-report-mermaid
  original-skill-name: markdown-mermaid-writing
  original-source: https://github.com/SuperiorByteWorks-LLC/agent-project
  original-license: Apache-2.0
---

# Security Report — Markdown and Mermaid Writing

## SecManus workspace (`read_file` paths)

Use this skill whenever the **final user-visible report** must be structured markdown (optionally with **Mermaid** diagrams): e.g. bodies under **`## SM_FULL_REPORT`**, markdown passed to the security workspace UI, or delegated subagent deliverables that the parent will merge into the report.

**Virtual paths (POSIX, use with `read_file` / `sread_file`):**

| Mount | When | Example |
| ------ | ---- | -------- |
| **`/skills-main/security-report-mermaid/...`** | Main DeepAgent — package is allowlisted in `config/main_agent_skills.yaml` | `/skills-main/security-report-mermaid/references/mermaid_style_guide.md` |
| **`/skills/security-report-mermaid/...`** | Subagents / dev when the full global skills tree is exposed | `/skills/security-report-mermaid/templates/status_report.md` |

Within this package, relative paths in the tables below (e.g. `references/diagrams/flowchart.md`) are under **`security-report-mermaid/`** — prefix with the mount you have access to.

Contract alignment:

- Do **not** break **MASTER_AGENT.md** rules for **`## SM_FULL_REPORT`** / **`## SM_TASK_DIGEST`** ordering and report-first layout.
- Security subagents: **`### SM_STATS_PAYLOAD`** and findings JSON stay **outside** the narrative per `SUBAGENT_OUTPUT_APPENDIX`; prose and Mermaid live in the normal report body.
- **Session language:** write diagram labels and prose in the same language as the user-facing turn when product policy calls for it.
- **GFM tables:** The workspace UI parses **GitHub Flavored Markdown** tables (`remark-gfm`). Emit **one table row per line** (pipe-separated). Include the header separator row `|---|---|` (adjust dashes/column count to match). **Do not** concatenate multiple data rows onto a single line or join rows only with stray `|` / `||` blobs — parsers will treat that as plain text. See **`references/markdown_style_guide.md`** for table conventions.
...
- Preparing **SecManus workspace** outward-facing summaries (attacks, IOC flows, timelines, architecture of compromise)
...
### SecManus security skills

Combine with **`general-security`** / **`vuln-scan`** workflows: analysis tools produce findings; **this skill** structures the **Markdown + Mermaid** narrative for **`## SM_FULL_REPORT`**.
...
2. For every figure that shows a workflow, architecture, or relationship:
   - Replace placeholder with a Mermaid diagram following this skill's guides
...
| ----------------------- | ------------------------------------------- | -------------------------------------------------- |
| Markdown Style Guide | `references/markdown_style_guide.md` | Headings, formatting, citations, tables, Mermaid integration, templates, quality checklist |
| Mermaid Style Guide | `references/mermaid_style_guide.md` | Accessibility, emoji set, color classes, theme neutrality, type selection, complexity tiers |
...
Bundled **`references/`**, **`templates/`**, and **`assets/`** originate from **`SuperiorByteWorks-LLC/agent-project`** under the **Apache-2.0** License (`LICENSE`, `NOTICE.txt` in this directory).

- **Upstream source**: https://github.com/SuperiorByteWorks-LLC/agent-project
- **Author**: Clayton Young / Superior Byte Works, LLC (@borealBytes)

SecManus layers the **package name**, **SecManus workspace** section, and **`SKILL.md` frontmatter** on top of this bundle without claiming ownership of upstream files.
