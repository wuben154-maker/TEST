...
> **binary_analysis contract:** Background below mixes ATT&CK and Windows platform facts. The agent **must** only drive analysis with **ADR-13** host tools and `SandboxSessionTool` on samples; EDR, Sysmon, and hunt-query rows are for analyst mapping off-host and must not be implied as available tool calls in this skill.
