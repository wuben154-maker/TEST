...
## Runtime note (`binary_analysis`)

The phases below are **external** EDR, SIEM, and host-forensics playbooks for human operators. They are **not** the mandatory execution path of the `binary_analysis` agent, which is bounded by the active run’s `contracts.md` and must not treat Splunk, MDE, pe-sieve, or Volatility on the **host** as first-class tools. When this file is opened from `SKILL.md`, treat it as **optional** background; chain writes follow Proto-02 / Proto-03 and **facts from allowed tooling** only.
