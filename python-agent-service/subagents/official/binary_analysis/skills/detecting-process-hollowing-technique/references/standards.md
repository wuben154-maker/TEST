...
The MITRE tables, API call sequences, and “Detection Data Sources” / **Volatility** sections below are **analyst** references and external DFIR context. The `binary_analysis` agent does **not** use Sysmon, MDE, or Volatility on the **host** as first-class tools; static chain writes follow the active run’s `audit-runs/<run_id>/contracts.md` and only **tool-backed** `imports` / `strings_iocs` / `disassembly` facts plus Proto-02 / Proto-03. Cross-check `SKILL.md` **Runtime Contract** before treating any table here as an agent step.
