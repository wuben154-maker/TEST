...
## Runtime boundary (binary_analysis)

This file is **reference material** for analysts and LLM reasoning. In the
`binary_analysis` runtime, **all** sample bytes and optional helper scripts
(`1768.py`, `parse_beacon_config.py`, `dissect.cobaltstrike`, YARA compiles,
JARM probes) run **only** under `/workspace/<analysis_id>/` via
`sandbox_session` with `bash` / `python_exec`—never via host `open()` or
unsandboxed subprocesses. Facts enter the chain through scheduled tools
(`string_extract`, `binary_view`, decompile outputs, and so on); long
artefacts stay out of raw LLM context unless wrapped per Proto-03
(`binary-analysis-sanitize-untrusted-strings`). Do not treat Wireshark,
Volatility, or live IDS deployment as first-class agent capabilities.
